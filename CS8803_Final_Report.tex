...
\subsection{First Instance Comparison}

To provide concrete insight into model behavior, we analyze how each approach handles a representative instance from the evaluation set. This instance involves fixing a type-checking bug in a Python utility function.

\textbf{Input Example}:

\begin{quote}
\small
\textbf{Instance ID}: ``pandas__core-bug-fix-001''

\textbf{Issue Description}: ``TypeError: cannot multiply sequence by non-int of type 'float' when processing cost calculations with mixed numeric types in utility functions''

\textbf{Retrieved Code Context} (top-3 chunks from FAISS):
\begin{verbatim}
def calculate_cost(items, multiplier=1.0):
...
\textbf{Model Outputs and Comparison}:

\begin{table}[t]
\caption{First instance comparison across all approaches}
\label{tab:first_instance}
\begin{center}
\begin{tabular}{lp{3cm}lll}
\hline
\textbf{Model} & \textbf{Patch Validity} & \textbf{Format} & \textbf{Applies} & \textbf{Correct} \\
\hline
Base (Qwen 0.5B) & Invalid format & ✗ & -- & -- \\
SFT (Qwen 0.5B) & Valid & ✓ & ✓ & ✗ \\
GRPO (Qwen 0.5B) & Valid & ✓ & ✓ & ✓ \\
Llama 1B & Valid & ✓ & ✓ & ✗ \\
Llama 3B & Valid & ✓ & ✓ & ✓ \\
\hline
\end{tabular}
...
\textbf{Base Model (Qwen 0.5B) - Failed}:
The untrained base model struggles to produce proper SEARCH/REPLACE format, generating pseudo-code instead of actionable edits:
\begin{quote}
\small
\textit{Output}: ``To fix this, use isinstance checks: if isinstance(multiplier, float)...''
\end{quote}

\textbf{SFT Model (Qwen 0.5B) - Partial Success}:
After supervised fine-tuning, the model produces valid format but incorrect logic:
\begin{quote}
\small
\begin{verbatim}
<<<<<<< SEARCH
total = total + item * multiplier
=======
total = total + float(item) * float(multiplier)
>>>>>>> REPLACE
\end{verbatim}

This patch applies cleanly but fails because it doesn't address the root cause (string items causing the error).
\end{quote}

\textbf{GRPO Model (Qwen 0.5B) - Correct Fix}:
After RL training, the model identifies and fixes the true issue:
\begin{quote}
\small
\begin{verbatim}
<<<<<<< SEARCH
...
The GRPO model correctly identifies the multi-step fix needed: convert both operands to float before multiplication.
\end{quote}

\textbf{Llama 1B (Zero-shot) - Partial Success}:
Produces valid format but with insufficient reasoning:
\begin{quote}
\small
\begin{verbatim}
<<<<<<< SEARCH
...
While syntactically valid, this refactoring doesn't address the type conversion issue.
\end{quote}

\textbf{Llama 3B (Zero-shot) - Correct Fix}:
Despite zero-shot setup, the larger model produces a complete fix:
\begin{quote}
\small
\begin{verbatim}
<<<<<<< SEARCH
...
=======
for item in items:
    total = total + float(item) * multiplier
>>>>>>> REPLACE
\end{verbatim}

The fix correctly converts the variable item to float, addressing the TypeError without unnecessary conversions.
\end{quote}

\textbf{Key Insights from First Instance}:

\begin{enumerate}
    \item \textbf{Training improves reasoning}: Base → SFT shows format improvement; SFT → GRPO shows correctness improvement. The instance demonstrates RL's value in moving from syntactically valid to semantically correct patches.

    \item \textbf{Model scale matters}: Llama 3B's zero-shot fix matches GRPO's quality without training. This suggests that for many instances, larger pre-trained models already encode sufficient reasoning capability.

    \item \textbf{Different failure modes}: SFT and Llama 1B both produce valid but incomplete fixes, while Base Model fails at format. This suggests that format and semantic reasoning are separable concerns.

    \item \textbf{Efficiency insights}: Llama 1B's fix, while incorrect, demonstrates that smaller models can at least grasp the basic structure and produce defensible (if incomplete) solutions.
\end{enumerate}
...
\begin{quote}
\small
You are a code repair expert. Your task is to fix a software bug given an issue description and relevant code context.

\textbf{Instructions}:
\begin{enumerate}
    \item Carefully read and analyze the issue description
    \item Review the provided code context to understand the codebase
    \item Identify the root cause of the bug
    \item Generate SEARCH/REPLACE edits to fix the issue
    \item Ensure your edits are minimal and focused on the bug fix
    \item Each SEARCH block must match existing code exactly (including whitespace)
    \item Format all edits using the SEARCH/REPLACE syntax shown below
\end{enumerate}

\textbf{Output Format}: Use exactly this format for each code edit:
...
\textbf{Code Context} (retrieved most relevant code chunks):
[top-k code chunks retrieved via FAISS semantic search]

\textbf{Task}: Generate SEARCH/REPLACE edits to fix the issue described above. Ensure:
\begin{itemize}
    \item Each SEARCH block matches code exactly as it appears in the files above
    \item Edits are minimal and focused on the root cause
    \item The resulting code is syntactically valid Python
    \item No new bugs or linting errors are introduced
\end{itemize}
\end{quote}
...
\textbf{Issue Description}:
``When processing lists with mixed types, the normalize\_values function raises TypeError: cannot multiply sequence by non-int of type 'float'. The function should handle both string and numeric list items gracefully.''

\textbf{Code Context}:

\begin{verbatim}
# From utils/helpers.py
def normalize_values(data, scale=1.0):
    \"\"\"Normalize values in a list by scaling factor.\"\"\"
    result = []
    for item in data:
...
# Usage in main.py
values = ['100', '200', 300]
normalized = normalize_values(values, scale=0.5)
\end{verbatim}

\textbf{Task}: Generate SEARCH/REPLACE edits to fix the TypeError when processing mixed-type lists.
...
def normalize_values(data, scale=1.0):
    ...
    result = []
    for item in data:
        # Convert to float to handle both strings and numbers
        item_val = float(item) if isinstance(item, str) else item
        result.append(item_val * scale)
    return result
>>>>>>> REPLACE
```
\end{verbatim}
